...
for filepath in all_article_files:
    ...
    # Find the start of any responsive section in the <style> block and replace all the way to </style>
    pattern = r'(@media\s*\(max-width:\s*(1000px|1024px|900px|640px)\)|/\*\s*═══════════════════════════════════════════════\s*RESPONSIVE & MOBILE POLISH).*?(?=</style>)'
    if re.search(pattern, content, flags=re.DOTALL):
        content = re.sub(pattern, article_mobile_css.strip() + "\n  ", content, flags=re.DOTALL)
...
# Also clean up en/blog/index.html if needed
with open("en/blog/index.html", "r", encoding="utf-8") as f:
    idx = f.read()
idx = re.sub(r'@media\s*\(max-width:\s*640px\)\s*\{.*?\}', '', idx, flags=re.DOTALL)
