...
# Read the UI dictionary and article metadata
from rebuild_all_multilingual_articles_master import ui_translations, cta_content, articles_meta, master_article_css
...
for filename in articles_meta.keys():
    ...
    # Extract all HTML inside <article class="article-body"> ... </article>
    m = re.search(r'<article class="article-body">(.*?)</article>', html, flags=re.DOTALL)
    if not m:
        # Check if inside <main ...> ... </main>
    ...
    if m:
        ...
        # If placeholder exists, split around it
        ...
        print(f"Extracted {filename}: Part 1 ({len(clean_articles_content[filename]['part1'])} chars), Part 2 ({len(clean_articles_content[filename]['part2'])} chars)")
    else:
