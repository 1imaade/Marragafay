...
# Extract clean pure semantic content without any wrapper tags
...
for filename in articles_meta.keys():
    ...
    if m:
        ...
        # Strip all nested article and div wrappers
        ...
        clean = re.sub(r'<div class="article-experience-inset">.*?</div>\s*</div>?', '', clean, flags=re.DOTALL)
        clean = re.sub(r'</article>', '', clean)
        clean = re.sub(r'</div>\s*$', '', clean.strip())
        clean_articles_content[filename] = clean.strip()
        print(f"Extracted clean {filename}: {len(clean_articles_content[filename])} chars")
...
def generate_article_page(filename, lang):
    ...
    nav = t["nav_links"]
    css_block = clean_css + (rtl_style if is_rtl else "")
...
<body class="blog">
  ...
  <!-- ARTICLE BODY (SINGLE WRAPPER ONLY) -->
  <main>
...
print("Clean single wrapper rebuild complete across all 28 articles!")
