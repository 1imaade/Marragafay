...
new_hero = """        <div class="grid grid-cols-1 lg:grid-cols-3 gap-1 h-full w-full bg-white">
...
# Replace the specific grid inside the section
pattern = re.compile(r'        <div class="grid grid-cols-1 lg:grid-cols-12 h-full">[\s\S]*?</div>\s*</div>\s*</div>\s*</div>', re.MULTILINE)
