...
(function ($) {
	...
	// Only initialise Stellar parallax on non-mobile devices.
	// On mobile, resize/scroll events from Stellar can cause the page
	// to jump back to the hero section whenever the browser chrome
	// (address bar) appears or disappears.
	if (!isMobile.any() && $.fn.stellar) {
		$(window).stellar({
			responsive: true,
	...
	var fullHeight = function () {
		...
		if (isMobile.any()) {
			// On mobile, do NOT set an explicit pixel height.
			// The CSS rule `height: 100svh` (Small Viewport Height) handles
			// the hero sizing correctly without reacting to the browser
			// chrome (address bar) appearing / disappearing on scroll.
			// We only update on a genuine orientation / width change so that
			// landscape ↔ portrait transitions still work.
			var lastWidth = window.innerWidth;
			$(window).on('resize orientationchange', function () {
				var newWidth = window.innerWidth;
				if (newWidth !== lastWidth) {
					lastWidth = newWidth;
					// Remove inline height so the CSS svh value takes over again.
					$('.js-fullheight').css('height', '');
				}
			});
		} else {
			// Desktop: set explicit pixel height and update on any resize.
	...
	// isMobile is already declared at the top of this IIFE (line ~23).
	// The duplicate declaration has been removed to avoid shadowing.
	...
	if ($('.hero-bg-slider').length && $.fn.owlCarousel) {
